...
const Viewer = dynamic(
  ...
  { ssr: false }
) as React.ComponentType<{ src: string }>
...
export default function RobotPage() {
  ...
  return (
    <main className="container mx-auto px-4 py-10 text-white space-y-10">
      ...
      <div className="relative min-h-[320px] flex">
        <Image
          src="/images/meowtransparent.png"
          alt="Robot"
          height={320}
          width={1000}
        />
        <Image src="/images/render2.png" alt="Robot" height={320} width={600} />
      ...
      <header>
        ...
        <p className="text-white/80 mt-3">
          {t(
            "Technical showcase, evolution, and key innovations.",
            "技术展示、迭代演进与核心创新。"
          )}
        </p>
      </header>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <article className="rounded border border-white/10 bg-white/5 p-4">
            ...
            <p className="text-xs text-white/60 mt-2">
              {t(
                "Automation: Limelight 3A + color sensor enable closed-loop grabbing during TeleOp.",
                "自动化：Limelight 3A + 颜色传感器，在手动阶段实现闭环抓取。"
              )}
            </p>
          </article>
          ...
          <article className="rounded border border-white/10 bg-white/5 p-4">
            ...
            <p className="text-xs text-white/60 mt-2">
              {t(
                "Measured output ~5.25 N·m → ~154 N at 34 mm arm; meets 12 kg load with ~3.6× safety.",
                "测得输出约 5.25 N·m → 臂长 34 mm 约 154 N；满足 12 kg 载荷并具 ~3.6× 安全系数。"
              )}
            </p>
          </article>
          ...
          <article className="rounded border border-white/10 bg-white/5 p-4">
            ...
            <p className="text-xs text-white/60 mt-2">
              {t(
                "Average alignment ~0.5 s; ~92% grasp success in tests.",
                "平均对准约 0.5 秒；测试中抓取成功率约 92%。"
              )}
            </p>
          </article>
          ...
          <article className="rounded border border-white/10 bg-white/5 p-4">
            ...
            <p className="text-xs text-white/60 mt-2">
              {t(
                "Reduces tuning cycles from many runs → single run with full telemetry.",
                "将多次调试缩减为一次完整遥测的回放分析。"
              )}
            </p>
          </article>
