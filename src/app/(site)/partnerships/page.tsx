  ...
  return (
    <main className="container mx-auto px-4 py-10 text-white space-y-10">
      <header className="space-y-2">
        <h1 className="text-4xl md:text-6xl font-extrabold">{t("Partnerships", "合作伙伴")}</h1>
  ...
  {t(
    ...
    "与创新同行，放大 STEM 影响力——展演、文档与外联，已触达 300+ 人。"
      ...
      <section className="space-y-4">
        <h2 className="text-2xl font-bold text-orange-400">{t("Sponsorship Opportunities", "赞助机会")}</h2>
        <ul className="grid md:grid-cols-3 gap-4">
{[
  { tier: t("Supporter", "支持者"), value: t("Logo on website and pit board; acknowledgment in resources.", "网站与 pit 展板展示 logo；资源页面致谢。") },
  { tier: t("Partner", "合作伙伴"), value: t("Logo, social media mentions, and event shout-outs; demo opportunities.", "Logo、社媒提及、活动鸣谢；活动演示机会。") },
  { tier: t("Title Sponsor", "冠名赞助"), value: t("Prominent logo placement, co-branded outreach, and on-site demos.", "显著 logo 露出、联合外联活动与现场演示。") }
].map((s, i) => (
...
<p className="text-sm text-white/80">
  {t(
    ...
    "我们期待机械（CV4B、PTO）、软件（命令式架构、AI 视觉）与策略导师加入我们的导师网络。"
      ...
      {/* Exhibition Services */}
      <section className="space-y-3">
        <h2 className="text-2xl font-bold text-orange-400">{t("Exhibition Services", "科普展演")}</h2>
        <ul className="grid md:grid-cols-3 gap-4">
{[
  t("STEM education presentations (school CS/Math week)", "STEM 教育宣讲（校内计算机/数学周）"),
  t("Robot demonstrations and hands-on sessions", "机器人演示与互动体验"),
  t("Workshops on design, control, and AI vision", "设计、控制与 AI 视觉工作坊")
].map((item, i) => (
...
<footer className="text-sm text-white/60">
  {t("Need a formal proposal? Email us for a sponsorship deck and collaboration options.", "需要正式方案？给我们发邮件获取赞助资料与合作方案。")}
</footer>
    </main>
  )
}
