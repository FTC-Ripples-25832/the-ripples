...
void main() {
  ...
  // Use robot image as mask for emission
  float scale = 0.2; // 2.0 = half size
  vec2 vUvScaled = (vUv - 0.5) / scale + 0.5;
  float robotMask = texture2D(uRobot, vUvScaled).a;
  // If the robot image is not RGBA, use brightness: float robotMask = dot(texture2D(uRobot, vUvScaled).rgb, vec3(0.299, 0.587, 0.114));
  if (robotMask > 0.2) {
    d = uTriangleHeight;
  }
